...
print('enter look ahead buffer size, if chosen to be larger than sliding window it will be reset to be equal sliding window.....')
look_ahead = int(input())
if look_ahead > sliding_window:
    look_ahead = sliding_window
...
encoded_tuple = np.array([],dtype=np.uint16)
...
#save encoding 
np.save('tuples.npy', encoded_tuple)
np.save('chars.npy', encoded_charachter)